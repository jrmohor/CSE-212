...
def summarize_degrees(text_info):
    """
    ...
    """
    degrees = dict()
    file_in = str(requests.get(text_info).content)
    # print(file_in[:300]) # Print first 300 characters for testing purposes
    for line in file_in.split('\\n'):
        fields = line.split(",") 
        degrees.update({fields[3]: ((degrees.get(fields[3]) or 0) + 1)})
        # ADD YOUR CODE HERE
...
# Sample Test Cases (may not be comprehensive) 
print("\n=========== PROBLEM 2 TESTS ===========")
print(summarize_degrees("https://byui-cse.github.io/cse212-course/lesson06/census.txt")) # You might need to add a path for the file
...
def is_anagram(word1, word2):
    ...
    # Check if lengths of words are different
    if len(word1) != len(word2):
        return False

    # Create dictionaries to count occurrences of each letter
    count1 = {}
    count2 = {}
    ...
    # Count occurrences of each letter in word2
    for char in word2:
...
class Maze:
    ...
    def board_moves(self,x,y,dir):
        if self.maze_map.get((self.curr_x, self.curr_y))[dir]:
            self.curr_x += x
            self.curr_y += y
        else:
            print("Stop!Can't go that way!")
...
def earthquake_daily_summary():
    ...
    for earthquake in earthquakes:
        ...
        print(f"Location: {place}, Magnitude: {round(magnitude,2)}".replace(' km ', 'km '))
